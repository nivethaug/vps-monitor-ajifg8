...
def restart_pm2(project_name: str = None):
    ...
    Args:
        project_name: Project name (uses vps-monitor-ajifg8 placeholder by default)
    """
    print("\n" + "="*50)
    print("PM2 RESTART")
    print("="*50)
    ...
    if not project_name:
        project_name = "vps-monitor-ajifg8"
    
    return run(f"sudo pm2 restart vps-monitor-ajifg8-frontend")
...
def main():
    ...
    if not args.no_restart and success:
        restart_pm2()  # Uses vps-monitor-ajifg8 placeholder
        reload_nginx()
